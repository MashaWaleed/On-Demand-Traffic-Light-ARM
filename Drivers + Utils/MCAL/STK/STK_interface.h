...
/**
 * @brief Set an unlimited delay function in milliseconds
 *
 * This function configures the SysTick timer to count and overflow repeatedly as much as wanted to accomplish the delay.
 *
 * @param[in] Copy_Milliseconds The duration of delay in microseconds.
 * @warning This function can be highly inaccurate at very long delays, good for a couple of minutes.
 * @return E_OK if the interval configuration was successful, E_NOT_OK if an error occurred.
 */
Std_ReturnType STK_SetDelayAny_ms(f32 Copy_Milliseconds);

/**
 * @brief Set an unlimited delay function in microseconds
 *
 * This function configures the SysTick timer to count and overflow repeatedly as much as wanted to accomplish the delay.
 *
 * @param[in] Copy_Microseconds The duration of delay in microseconds.
 * @warning This function can be highly inaccurate at very long delays, good for a couple of minutes.
 * @return E_OK if the interval configuration was successful, E_NOT_OK if an error occurred.
 */
